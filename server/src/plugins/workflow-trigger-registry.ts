...
import type { WorkflowTriggerSourceMeta } from '@my-claudia/shared';
...
class WorkflowTriggerSourceRegistry {
   ...
   * If one with the same id exists, it will be overwritten with a warning.
   */
  ...
  getByPlugin(pluginId: string): WorkflowTriggerSourceMeta[] {
    return Array.from(this.sources.values()).filter((s) => s.source === pluginId);
  }
