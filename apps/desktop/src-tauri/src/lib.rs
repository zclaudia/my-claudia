...
fn toggle_claudia_chat(
    ...
    if let Some(chat_window) = app.get_webview_window("claudia-chat") {
        let visible = chat_window.is_visible().map_err(|e| e.to_string())?;
        if visible {
            chat_window.hide().map_err(|e| e.to_string())?;
            ball_window.show().map_err(|e| e.to_string())?;
        } else {
            chat_window
                .set_position(Position::Logical(LogicalPosition::new(chat_x, chat_y)))
